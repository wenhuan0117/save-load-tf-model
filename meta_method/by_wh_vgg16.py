...
def check_img(test_data_path):
    ...
    with tf.Session() as sess:
       ...
       pre1=sess.run(pre,feed_dict={tfx:x_data})
    return pre1,image_path

##分类到不同的文件夹
def classifier(result,image_path):
    path1=test_data_path+'00'
    path2=test_data_path+'01'
    try:
        os.mkdir(path1)
        os.mkdir(path2)
    except Exception:
    ...
    for i in range(len(result)):
        if result[i]==0:
            try:
                shutil.copy(image_path[i],path1)
                print('try1')
            except Exception:
                pass
        else:
            try:
                shutil.copy(image_path[i],path2)
                print('try2')
            except Exception:
                pass    
##训练
...
##测试
result,image_path=check_img(test_data_path)

##分类            
classifier(result,image_path)
